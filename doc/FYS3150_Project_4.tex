...
Outside the loop of the temperatures, the double \verb+wtime+ is used to measure the amount of time it takes to run all the simulations. Before the simulations start the amound of threads allocated is printed to the terminal, and after they are finished the time spent is also printed to the terminal.


INSERT WRITETOFILEMULTI() CODE SNIPPET
...
%\subsection{Program usage} \label{sec:III:b}

%We implemented our solver as a class in C++ which we named \verb+IsingMetropolis+ as it solves the Ising model with the Metropolis algorithm. We also limit ourselves to a two-dimensional square lattice, but as naming goes we omitted including this in the name of the class. The class is defined in the header file \verb+ising_metropolis.hpp+ (see appendix \hyperref[A.1]{A.1}) with the functions belonging to it being defined in \verb+ising_metropolis.cpp+ (see appendix \hyperref[A.2]{A.2}). The program \verb+main.cpp+ (see appendix \hyperref[A.3}{A.3}) takes command line arguments and operates the simulation using these. Finally the Python script \verb+project.py+ automates all of the simulations that we perform, and processes the resulting data to produce the results shown later in this report. We will discuss these programs in more detail in the order outlined here. 


%\cprotect\subsubsection{A class to simulate the Ising model using the Metropolis algorithm} \label{sec:III:b:i}

%The \verb+IsingMetropolis+ class has several functions that we will cover here. First we must describe the two different ways in which a simulation can be run, as the split between these two is relevant throughout almost all of the programs. The class can either run a single simulation at a specified temperature, or it can run simulations at several temperatures in a single run. The differences between what the program does in these cases will be covered where it is relevant.

%There are two constructors belonging to the class, one that establishes the necessary variables in order to run the simulation at a single temperature, and one that does the same in case simulations for multiple temperatures are to be performed. The first constructor takes (in order) the number of spins in one direction in the lattice, the number of Monte Carlo cycles to perform, and the name of a file to write the results to. 
