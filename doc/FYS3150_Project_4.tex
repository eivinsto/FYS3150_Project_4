...
\subsubsection{Periodic \(L \times L\) square lattice with no external field} \label{sec:II:a:ii}
...
\begin{align*}
Z_N &= [2\cosh(\beta J) e^I ]^N \, , \numberthis \label{eq:LxL_partition_function}
\end{align*}
...
and $N$ is the number of particles. The resulting mean energy is:

\begin{align*}
\langle E \rangle &= -J \coth( 2\beta J) \bigg[ 1 + \frac{2}{\pi} (2\tanh^2 (2\beta J) - 1) K_1(\kappa) \bigg] \, , \numberthis \label{eq:LxL_energy}
\end{align*}
...
\subsection{Phase transitions in the Ising Model} \label{sec:II:b}
